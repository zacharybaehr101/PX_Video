...
def build_speaker_segment(speaker: dict, quote: dict, media_dir: str, scored_media: list,
                           ...
                           burn_captions: bool = True, narration_audio_path: str = None):
    """
    ...
    Two modes:
      - Talking-head (speaker has "clip"): facetime cutaway + lower third,
        then b-roll for the remainder, exactly as before.
      - Narration/audio-only (speaker has no "clip"): no facetime, no
        lower third - b-roll plays for the ENTIRE quote duration, audio
        sourced from narration_audio_path (the manifest's shared
        voice_audio_url) instead of a per-speaker video file.
    """
    timing = quote["timing"]
    if timing is None:
    ...
    has_clip = bool(speaker.get("clip"))

    if not has_clip:
        if not narration_audio_path or not os.path.exists(narration_audio_path):
            print(f"WARNING: speaker '{speaker.get('name', '?')}' has no clip and no narration audio found - skipping quote")
            return None
        return _build_narration_segment(speaker, quote, narration_audio_path, media_dir, scored_media,
                                         used_broll, frame_w, frame_h, brand, is_vertical, burn_captions,
                                         start, end, duration)
...
def _build_narration_segment(speaker: dict, quote: dict, narration_audio_path: str, media_dir: str,
                              scored_media: list, used_broll: set, frame_w: int, frame_h: int,
                              brand: dict, is_vertical: bool, burn_captions: bool,
                              start: float, end: float, duration: float):
    """
    Audio-only mode: no talking head, no lower third. B-roll cuts play for
    the entire quote duration, captions still burn in (they're the only
    on-screen text identifying what's being said, since there's no face).
    """
    narration_audio_full = AudioFileClip(narration_audio_path)

    video_segments, audio_segments = [], []
    broll_chunks = pick_broll_chunks(scored_media, used_broll, media_dir, duration)
    voice_cursor = start
    ...
    if broll_chunks:
        for path, media_type, chunk_dur in broll_chunks:
            ...
            chunk_audio = narration_audio_full.subclip(
                voice_cursor, min(voice_cursor + chunk_dur, narration_audio_full.duration)
            )
            audio_segments.append(chunk_audio)
            voice_cursor += chunk_dur
    else:
        # No b-roll at all - fall back to a solid brand card rather than
        # a blank/failed render (should be rare - flag if this fires often).
        print("WARNING: no b-roll available for narration segment - using solid brand card")
        solid = np.array(Image.new("RGBA", (frame_w, frame_h), (*hex_to_rgb(brand["title_card"]["bg_color"]), 255)))
        video_segments.append(ImageClip(solid).set_duration(duration))
        audio_segments.append(narration_audio_full.subclip(start, end))
...
def render_aspect_ratio(manifest: dict, aligned: dict, scored_media: list, media_dir: str,
    ...
    if manifest.get("voice_audio_url"):
        narration_audio_path = os.path.join(media_dir, "voice.mp3")
